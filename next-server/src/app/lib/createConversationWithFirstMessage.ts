 ...
 */
type CreateConversationWithFirstMessageResponse = {
  conversation: {
    id: string;
    name: string | null;
    isGroup: boolean;
...
export const createConversationWithFirstMessage = async ({
  ...
  return apiFetch<CreateConversationWithFirstMessageResponse>("/api/conversations/first-message", {
    method: "POST",
    body: JSON.stringify({
      userId,
      isGroup,
